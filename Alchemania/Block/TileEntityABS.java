...
public class TileEntityABS extends TileEntityBrewingStand
{
    ...
    private boolean canBrew()
    {
        if (this.brewingItemStacks[3] != null && this.brewingItemStacks[3].stackSize > 0)
        {
            ...
            if(var1.itemID == Alchemania.pill.shiftedIndex)
            {
            	for(ItemStack is : this.brewingItemStacks)
            		if(is!=null && is.itemID == Item.potion.shiftedIndex && Item.potion.getEffects(is) == null)
            			return true;
            	return false;
            }
            else if (!Item.itemsList[var1.itemID].isPotionIngredient())
            {
                return false;
            } 
            else
    ...
    private void brewPotions()
    {
        if (this.canBrew())
        {
            ...
            if(var1.itemID == Alchemania.pill.shiftedIndex)
            {
            	AlchemaniaEffect[] effs = AlchemaniaEffect.parseEffects(var1);
            	for(AlchemaniaEffect ae:effs)ae.solute();
            	
            	for (int var2 = 0; var2 < 3; ++var2)
            	{
            		if (this.brewingItemStacks[var2] != null && this.brewingItemStacks[var2].itemID == Item.potion.shiftedIndex)
            		{
            			brewingItemStacks[var2] = new ItemStack(Alchemania.solution);
            			AlchemaniaEffect.writeEffects(brewingItemStacks[var2], effs);
            		}
            	}
